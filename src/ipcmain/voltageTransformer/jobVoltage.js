...
import {voltageTransJobFunc, conditionFunc, attachmentFunc} from "@/function"
const pathUpload = path.join(__dirname, `/../attachment`)
var fs = require('fs')
import path from 'path'

export const saveJobVoltage = () => {
    ipcMain.handle('saveJobVoltage', async function (event, job) {
        try {
            let check = await voltageTransJobFunc.getJobById(job.id)
            if(check == undefined) {
                await voltageTransJobFunc.insertJob(job.asset_id, job)
            }
            else {
...
export const saveTestVoltage = () => {
    ipcMain.handle('saveTestVoltage', async function (event, testList, job_id) {
        try {
            let jobTest = await voltageTransJobFunc.getTestByJobId(job_id)
            ...
            if(listDelete.length != 0) {
                for(let i in listDelete) {
                    voltageTransJobFunc.deleteVoltageTransTest(listDelete[i].id)
            ...
            for(let i in listInsert) {
                ...
                await voltageTransJobFunc.insertTest(listInsert[i].job_id, listInsert[i])
            }
            for(let i in listUpdate) {
                ...
                await voltageTransJobFunc.updateTest(listUpdate[i])
            }
            return {
                success: true,
                message: "",
...
export const getTestVoltageByJobId = () => {
    ipcMain.handle('getTestVoltageByJobId', async function (event, job_id) {
        try {
            let data = await voltageTransJobFunc.getTestByJobId(job_id)
            return {
                success: true,
                data : data
            }
