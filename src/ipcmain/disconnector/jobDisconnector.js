...
import {disconnectorJobFunc, conditionFunc, attachmentFunc} from "@/function"
const pathUpload = path.join(__dirname, `/../attachment`)
var fs = require('fs')
import path from 'path'

export const saveJobDisconnector = () => {
    ipcMain.handle('saveJobDisconnector', async function (event, job) {
        try {
            let check = await disconnectorJobFunc.getJobById(job.id)
            if(check == undefined) {
                await disconnectorJobFunc.insertJob(job.asset_id, job)
            }
            else {
                await disconnectorJobFunc.updateJobVoltageTrans(job)
            }
            return {
                success: true,
                message: ""
...
export const saveTestDisconnector = () => {
    ipcMain.handle('saveTestDisconnector', async function (event, testList, job_id) {
        try {
            let jobTest = await disconnectorJobFunc.getTestByJobId(job_id)
            ...
            if(listDelete.length != 0) {
                for(let i in listDelete) {
                    disconnectorJobFunc.deleteDisconnectorTest(listDelete[i].id)
            ...
            for(let i in listInsert) {
                ...
                await disconnectorJobFunc.insertTest(listInsert[i].job_id, listInsert[i])
            }
            for(let i in listUpdate) {
                ...
                await disconnectorJobFunc.updateTest(listUpdate[i])
            }
            return {
                success: true,
                message: "",
...
export const getTestDisconnectorByJobId = () => {
    ipcMain.handle('getTestDisconnectorByJobId', async function (event, job_id) {
        try {
            let data = await disconnectorJobFunc.getTestByJobId(job_id)
            return {
                success: true,
                data : data
            }
