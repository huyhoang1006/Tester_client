...
<script>
/* eslint-disable */
import { ASSET_TYPE_TO_KEY, CATEGORY_OPTION, FEATURE_TREE } from './constants.js'
...
export default {
  ...
  methods: {
    async loadTemplates() {
      try {
        const rs = await window.electronAPI.getAllTemplatesByType(this.exportType)
    ...
    async confirmAddTemplate() {
      this.$refs.addForm.validate(async (valid) => {
        ...
        try {
          ...
          const rs = await window.electronAPI.insertTemplate({ name: this.addForm.name, path: this.addForm.filePath||'', variable: JSON.stringify([]), type: this.exportType || '' })
          if (rs?.success) {
    ...
    async saveWithScan() {
      ...
      try {
        const rs = await window.electronAPI.saveTemplateWithScan({ name: this.selectedTemplateName, filePath: this.currentFilePath, variables, type: this.exportType})
