...
import disconnectorAssessmentMap from "@/config/testing-assessment/Disconnector";

// Map measurement_id -> unit (gộp test-definitions + testing-condition của Disconnector)
const DC_UNIT_BY_MEASUREMENT = (() => {
    const map = {}
...
const unitOf = (measurementId) => (measurementId && DC_UNIT_BY_MEASUREMENT[measurementId]) || ''
...
export const jobDtoToEntity = (dto) => {
    ...
    for(const disconnectorTestingEquipmentTestType of dto.disconnectorTestingEquipmentTestType) {
        ...
        data.work_task_id = disconnectorTestingEquipmentTestType.work_task_id || null;
...
export const JobEntityToDto = (entity) => {
    ...
    for(const testingEquipment of entity.testingEquipment) {
        ...
        for(const disconnectorTestingEquipmentTestType of entity.disconnectorTestingEquipmentTestType) {
            if(disconnectorTestingEquipmentTestType.testing_equipment_id === data.mrid) {
                data.work_task_ids.push(disconnectorTestingEquipmentTestType.work_task_id);
            }
        }
    ...
    for (const item of entity.workTasks) {
        let condition = commonFunc.buildEmptyTestCondition(disconnectorConditionMap[item.type]?.columns || []);
        const testAssessmentList = JSON.parse(JSON.stringify(disconnectorAssessmentMap[item.type]?.testStandard || []));
