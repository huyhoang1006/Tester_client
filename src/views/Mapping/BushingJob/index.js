...
export const JobEntityToDto = (entity) => {
    ...
    for (const item of entity.workTasks) {
        let condition = commonFunc.buildEmptyTestCondition(bushingConditionMap[item.type]?.columns || []);
        const testAssessmentList = JSON.parse(JSON.stringify(bushingAssessmentMap[item.type].testStandard || []));
