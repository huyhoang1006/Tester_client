...
// Map measurement_id -> unit (gộp từ test-definitions + testing-condition của CT)
// Dùng để điền unit khi build cell (tránh unit rỗng).
const CT_UNIT_BY_MEASUREMENT = (() => {
    const map = {}
...
const unitOf = (measurementId) => (measurementId && CT_UNIT_BY_MEASUREMENT[measurementId]) || ''
...
export const jobDtoToEntity = (dto) => {
    ...
    try {
        ...
        for (const currentTransformerTestingEquipmentTestType of dto.currentTransformerTestingEquipmentTestType) {
            ...
            data.work_task_id = currentTransformerTestingEquipmentTestType.work_task_id || null;
    ...
    for (const testingEquipment of entity.testingEquipment) {
        ...
        for (const currentTransformerTestingEquipmentTestType of entity.currentTransformerTestingEquipmentTestType) {
            if (currentTransformerTestingEquipmentTestType.testing_equipment_id === data.mrid) {
                data.work_task_ids.push(currentTransformerTestingEquipmentTestType.work_task_id);
            }
        }
        dto.testingEquipmentData.push(data);
    }
    dto.currentTransformerTestingEquipmentTestType = entity.currentTransformerTestingEquipmentTestType || [];
        ...
        for(const testAssessment of testAssessmentList) {
            if(testAssessment.type == 'customized') {
                if (standardCustomized) {
                    ...
                    // fix parent_id sau khi đã có groupMap đầy đủ (group con trỏ tới group cha)
