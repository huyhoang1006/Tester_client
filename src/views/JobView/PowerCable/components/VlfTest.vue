...
export default {
    ...
    computed: {
        ...
        rowData()      { return common.buildEmptyTestRow(powerCableTestMap['VlfTest'].columns) },
    ...
    methods: {
        ensureConditionCell(code) {
            if (!this.conditions[code]) {
                const col = (powerCableConditionMap.VlfTest.columns || []).find(item => item.code === code) || {}
...
.result-col { flex-shrink: 0; width: 168px; text-align: center; border-left: 1px solid #ebeef5; padding: 0 8px; align-self: stretch; display: flex; align-items: center; justify-content: center; }
