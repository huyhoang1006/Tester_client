...
export default {
    ...
    async beforeMount() {
        loader.loaderStart()
        await this.getLocationAssetByIdDisconnector()
        this.mode = this.$route.query.mode
        if (this.mode === this.$constant.EDIT || this.mode === this.$constant.DUP) {
            this.job_id = this.$route.query.job_id
            const rs = await window.electronAPI.getJobDisconnectorById(this.job_id)
            console.log(rs)
            if (rs.success) {
                const data = rs.data
    ...
    methods: {
        async getLocationAssetByIdDisconnector() {
            const assetId = this.selectedAsset[0].id
            const rs = await window.electronAPI.getLocationAssetByIdDisconnector(assetId)
            if (rs.success) {
                const data = rs.data
        ...
        async saveJob() {
            if (this.mode === this.$constant.ADD || this.mode === this.$constant.DUP) {
                ...
                this.updateJobDisconnector()
            }
        },
        async insertJobdata() {
            loader.loaderStart()
            const rs = await window.electronAPI.insertJobDisconnector(
        ...
        async updateJobDisconnector() {
            loader.loaderStart()
            const rs = await window.electronAPI.updateJobDisconnector(this.properties, this.testList, this.testconditionArr, this.attachmentArr)
            if (rs.success) {
                this.$message({
                    type: 'success',
