...
export default {
    ...
    async beforeMount() {
        loader.loaderStart()
        await this.getLocationAssetByIdCurrentVoltage()
        this.mode = this.$route.query.mode
        if (this.mode === this.$constant.EDIT || this.mode === this.$constant.DUP) {
            this.job_id = this.$route.query.job_id
            const rs = await window.electronAPI.getJobCurrentVoltageById(this.job_id)
            if (rs.success) {
                const data = rs.data
    ...
    methods: {
        async getLocationAssetByIdCurrentVoltage() {
            const assetId = this.selectedAsset[0].id
            const rs = await window.electronAPI.getLocationAssetByIdCurrentVoltage(assetId)
            if (rs.success) {
                const data = rs.data
        ...
        async saveJob() {
            if (this.mode === this.$constant.ADD || this.mode === this.$constant.DUP) {
                ...
                this.updateJobCurrentVoltage()
            }
        },
        async insertJobdata() {
            loader.loaderStart()
            const rs = await window.electronAPI.insertJobCurrentVoltage(
        ...
        async updateJobCurrentVoltage() {
            loader.loaderStart()
            const rs = await window.electronAPI.updateJobCurrentVoltage(this.properties, this.testList, this.testconditionArr, this.attachmentArr)
            if (rs.success) {
                this.$message({
                    type: 'success',
