...
export default {
    ...
    methods: {
        async saveJob() {
            const validation = validateTestingEquipmentRows(this.transformerJobDto.testingEquipmentData, this.$constant.ROOT)
            if (!validation.valid) {
        ...
        checkTestingEquipment(data) {
            ...
            for (const transformer of arr) {
                const existed = data.transformerTestingEquipmentTestType.some(
                    old =>
                        old.testing_equipment_id === transformer.testing_equipment_id &&
                        old.work_task_id === transformer.work_task_id
                );
                if (!existed) {
                    data.transformerTestingEquipmentTestType.push(transformer);
                }
            }
