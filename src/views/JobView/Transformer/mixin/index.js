...
export default {
    ...
    methods: {
        async getLocationAsset() {
            const assetId = this.selectedAsset[0].id
            const rs = await window.electronAPI.getLocationAssetById(assetId)
            if (rs.success) {
                const data = rs.data
                const {asset, location, tapChangers, bushing} = data
                this.location = location
                this.asset = asset
                Object.keys(bushing).forEach(element => {
                    try {
                        bushing[element] = JSON.parse(bushing[element])
                    } catch(err) {
                ...
                const voltage_table = JSON.parse(tapChangers.voltage_table)
                this.tapChangers = {
                    ...tapChangers,
                    voltage_table
                }
        ...
        async insertJob() {
            loader.loaderStart()
            const rs = await window.electronAPI.insertJob(
        ...
        async updateJob() {
            loader.loaderStart()
            const rs = await window.electronAPI.updateJob(this.properties, this.testList, this.testconditionArr, this.attachmentArr)
            if (rs.success) {
                this.$message({
                    type: 'success',
