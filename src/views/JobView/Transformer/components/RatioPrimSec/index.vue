<template>
    <div id="ratio-prim-sec" class="test-ui">
        ...
        <table class="table-strip-input-data test-table">
                ...
                <tbody>
                    <tr>
                        <td>Ratio dev (%) ≤ <el-input size="mini" class="w-100px"
    ...
    methods: {
        ...
        async calcAssessment() {
            ...
            for (var i = 0; i < rows.length; i++) {
                ...
                if (row.ratio_dev && row.ratio_dev.measurement_id !== undefined) {
                    var _abs_ratio_dev = parseFloat(measurementMap[row.ratio_dev.measurement_id])
                    if (!isNaN(_abs_ratio_dev)) measurementMap[row.ratio_dev.measurement_id] = Math.abs(_abs_ratio_dev)
                }
