...
export default {
    ...
    methods: {
        async saveJob() {
            const validation = validateTestingEquipmentRows(this.bushingJobDto.testingEquipmentData, this.$constant.ROOT)
            if (!validation.valid) {
        ...
        checkTestingEquipment(data) {
            ...
            for (const bushing of arr) {
                const existed = data.bushingTestingEquipmentTestType.some(
                    old =>
                        old.testing_equipment_id === bushing.testing_equipment_id &&
                        old.work_task_id === bushing.work_task_id
                );
                if (!existed) {
                    data.bushingTestingEquipmentTestType.push(bushing);
                }
            }

            // Xóa các phần tử quá khứ không còn trong hiện tại
            data.bushingTestingEquipmentTestType = data.bushingTestingEquipmentTestType.filter(
                old => arr.some(
                    bushing =>
                        old.testing_equipment_id === bushing.testing_equipment_id &&
                        old.work_task_id === bushing.work_task_id
