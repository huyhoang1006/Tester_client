...
export default {
    ...
    methods: {
        async saveJob() {
            const validation = validateTestingEquipmentRows(this.surgeArresterJobDto.testingEquipmentData, this.$constant.ROOT)
            if (!validation.valid) {
            ...
            const terminalValidation = validateTerminalRows(
                this.surgeArresterJobDto.testList, 'InsulationResistance', () => buildPhaseTerminals()
            )
            ...
            for (const surge of arr) {
                const existed = data.surgeArresterTestingEquipmentTestType.some(
                    old =>
                        ...
                        old.work_task_id === surge.work_task_id
                );
                if (!existed) {
