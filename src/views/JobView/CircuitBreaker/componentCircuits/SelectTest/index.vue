...
export default {
    ...
    async beforeMount() {
        if(this.selectedAsset[0].asset === 'Transformer') {
            ...
            await this.getTestCircuitTypes()
        }
    },
    mounted() {},
    computed: {
    ...
    methods: {
        async getTestCircuitTypes() {
            const rs = await window.electronAPI.getTestCircuitTypes()
            if (rs.success) {
                const data = rs.data
        ...
        async getTestTypes() {
            const rs = await window.electronAPI.getTestTypes()
            if (rs.success) {
                const data = rs.data
                this.testTypeList = data
            } else {
            ...
            this.$confirm('This will permanently delete the file. Continue?', 'Warning', {
                ...
                .then(async () => {
                    ...
                    if (testId != this.$uuid.EMPTY) {
                        await window.electronAPI.deleteCircuitTest(testId)
                    }
