                        ...
                        <tr v-for="(item, index) in tableData" :key="index">
                            ...
                            <td>
                                <el-input size="mini" type="text" number="positive"
                                    v-model="item.closing_time.value" @input="calculateTimingSynchronism"></el-input>
                            </td>
                            <td v-if="index % (getInterruptersPerPhase() * getNumberOfPhases()) === 0"
                                ...
                                <el-input :rows="getInterruptersPerPhase() * getNumberOfPhases()" type="textarea"
                                    ...
                                    v-model="item.closing_sync_between_phase.value" readonly></el-input>
                            </td>
                            <td>
                                <el-input type="text" number="positive" size="mini"
                                    v-model="item.close_open_time.value"></el-input>
                            </td>
                            <td>
                    ...
                    <tbody>
                        <tr v-for="(item, index) in tableData" :key="index">
                            ...
                            <td v-if="index % (getInterruptersPerPhase() * getNumberOfPhases()) === 0"
                                :rowspan="getInterruptersPerPhase() * getNumberOfPhases()">
                                <el-input :rows="getInterruptersPerPhase() * getNumberOfPhases()" type="textarea"
                                    number="positive" v-model="item.closing_sync_between_interrupter.value" readonly></el-input>
                            </td>
                            <td>
                                <el-input size="mini" type="text" number="positive"
                                    v-model="item.close_open_time.value"></el-input>
            ...
            entries.forEach(function (entry) {
                ...
                rows.forEach(function (e, index) {
                    ...
                    if (nPhases > 1 && index % (iPerPhase * nPhases) === 0) {
                        results.push(this.assessTiming(e.closing_sync_between_phase ? e.closing_sync_between_phase.value : '', 5))
                    }
                    // closing_sync_between_interrupter [4]
                    if (iPerPhase > 1 && index % iPerPhase === 0) {
                        results.push(this.assessTiming(e.closing_sync_between_interrupter ? e.closing_sync_between_interrupter.value : '', 4))
                    }
                    // closing_time [3]
                    results.push(this.assessTiming(e.closing_time ? e.closing_time.value : '', 3))
                    // close_open_time [8]
                    results.push(this.assessTiming(e.close_open_time ? e.close_open_time.value : '', 8))
