...
export const getDinStandardById = async (mrid) => {
    return new Promise((resolve, reject) => {
        db.get(
            `SELECT * FROM din_standard WHERE mrid=?`,
            [mrid],
            (err, row) => {
...
export const insertDinStandardTransaction = async (data, dbsql) => {
...
// Xóa dinStandard
export const deleteDinStandardByIdTransaction = async (mrid, dbsql) => {

    // optional → không cần check
    await runPromise(
        dbsql,
        "DELETE FROM din_standard WHERE mrid=?",
        [mrid]
    )
