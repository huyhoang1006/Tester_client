...
export const getEpaStandardById = async (mrid) => {
    return new Promise((resolve, reject) => {
        db.get(
            `SELECT * FROM epa_standard WHERE mrid=?`,
            [mrid],
            (err, row) => {
...
export const insertEpaStandardTransaction = async (data, dbsql) => {
...
// Xóa epaStandard
export const deleteEpaStandardByIdTransaction = async (mrid, dbsql) => {

    // optional → không cần check
    await runPromise(
        dbsql,
        "DELETE FROM epa_standard WHERE mrid=?",
        [mrid]
    )
