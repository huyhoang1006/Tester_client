...
export const getJobCurrentVoltage = (assetId) => {
    return new Promise((resolve, reject) => {
        db.all("SELECT * FROM jobsCurrent where asset_id=?", [assetId], (err, rows) => {
            if (err) reject(err)
            resolve(rows)
        })
    })
...
export const getJobIds = (assetId) => {
    return new Promise((resolve, reject) => {
        db.all("SELECT id as id FROM jobsCurrent where asset_id=?", [assetId], (err, rows) => {
            if (err) reject(err)
            resolve(rows)
        })
    })
...
export const getTestByType = (test_id, type_id) => {
    return new Promise((resolve, reject) => {
        db.all("SELECT data FROM testsCurrent where id=? and type_id=?", [test_id, type_id], (err, rows) => {
            if (err) reject(err)
            resolve(rows)
        })
    })
...
export const checkJobExist = (properties) => {
    return new Promise((resolve, reject) => {
        db.get('SELECT * FROM jobsCurrent where name = ? and id != ? and asset_id = ? ',
...
export const insertJob = (assetId, properties) => {
    ...
    return new Promise((resolve, reject) => {
        db.run(
            'INSERT INTO jobsCurrent(id, asset_id, name, work_order, creation_date, execution_date, tested_by, approved_by, approval_date, summary, ambient_condition, testing_method, standard, average_health_index, worst_health_index)' +
                ' VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
            [
                id,
                assetId,
...
export const insertTest = (jobId, test) => {
    ...
    return new Promise((resolve, reject) => {
        db.run(
            'INSERT INTO testsCurrent(id, type_id, job_id, name, data, average_score, worst_score, total_average_score, total_worst_score, weighting_factor, worst_score_df, worst_score_c, average_score_df, average_score_c, created_on, weighting_factor_df, weighting_factor_c)' +
                ' VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
            [
                id,
                test.testTypeId,
...
export const deleteJob = (id) => {
    return new Promise((resolve, reject) => {
        db.run("DELETE FROM jobsCurrent WHERE id = ?", [id], (err) => {
            if (err) reject(err)
            resolve(true)
        })
    })
...
export const deleteCurrentVoltageTest = (id) => {
    return new Promise((resolve, reject) => {
        db.run("DELETE FROM testsCurrent WHERE id = ?", [id], (err) => {
            if (err) reject(err)
            resolve(true)
        })
    })
...
export const getJobById = (id) => {
    return new Promise((resolve, reject) => {
        db.get("SELECT * FROM jobsCurrent where id = ?", [id], (err, row) => {
            if (err) reject(err)
            resolve(row)
        })
    })
...
export const getTestByJobId = (jobId) => {
    return new Promise((resolve, reject) => {
        db.all(
            'select testsCurrent.id, testsCurrent.name, testsCurrent.data, testsCurrent.average_score, testsCurrent.worst_score, testsCurrent.worst_score_df, testsCurrent.worst_score_c, testsCurrent.average_score_df, testsCurrent.average_score_c , testsCurrent.total_average_score, testsCurrent.total_worst_score, testsCurrent.weighting_factor, testsCurrent.weighting_factor_df, testsCurrent.weighting_factor_c, testCurrent_type.id as testTypeId, testCurrent_type.code as testTypeCode, testCurrent_type.name as testTypeName, testsCurrent.created_on from testsCurrent, testCurrent_type WHERE testsCurrent.job_id = ? and testsCurrent.type_id = testCurrent_type.id order by testsCurrent.created_on asc',
            [jobId],
            (err, rows) => {
                if (err) reject(err)
                resolve(rows)
...
export const getAllTestByJobId = (jobId) => {
    return new Promise((resolve, reject) => {
        db.all(
            'SELECT * FROM testsCurrent WHERE job_id = ?',
            [jobId],
            (err, rows) => {
                if (err) reject(err)
                resolve(rows)
...
export const getTestIdByJobId = (jobId) => {
    return new Promise((resolve, reject) => {
        db.all("SELECT id FROM testsCurrent where job_id = ?", [jobId], (err, rows) => {
            if (err) reject(err)
            resolve(rows)
        })
    })
...
export const updateJobCurrentVoltage = (job) => {
    return new Promise((resolve, reject) => {
        db.run('UPDATE jobsCurrent' +
...
export const checkJobExistByName = (name, id) => {
    return new Promise((resolve, reject) => {
        db.get('SELECT * FROM jobsCurrent where name = ? and asset_id = ?',
            [name, id], (err, row) => {
                if (err) {
                    reject(err)
                } else {
...
export const getTestTypeByCode = (code) => {
    return new Promise((resolve, reject) => {
        db.get("SELECT * FROM testCurrent_type where code = ?", [code], (err, row) => {
            if (err) reject(err)
            resolve(row)
        })
    })
...
export const getTestTypeById = (id) => {
    return new Promise((resolve, reject) => {
        db.get("SELECT * FROM testCurrent_type where id = ?", [id], (err, row) => {
            if (err) reject(err)
            resolve(row)
        })
    })
