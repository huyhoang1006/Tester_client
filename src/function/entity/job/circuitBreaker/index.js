...
export const insertCircuitBreakerJobEntity = async (old_entity,entity) => {
    try {
        ...
        return {
            success: false,
            error,
            message: (error && error.message) || 'Error saving Circuit Breaker job entity'
        };
    }
}
        ...
        if(id == null || id === '') {
            ...
            if(dataOldWork.success) {
                ...
                for(const equipment of entity.testingEquipment) {
                    const dataEquipmentTestType = await getCircuitBreakerTestingEquipmentTestingEqId(equipment.mrid);
                    if(dataEquipmentTestType.success) {
                        entity.circuitBreakerTestingEquipmentTestType = entity.circuitBreakerTestingEquipmentTestType.concat(dataEquipmentTestType.data);
                    }
                }
