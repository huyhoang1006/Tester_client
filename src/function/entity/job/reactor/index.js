...
export const insertReactorJobEntity = async (old_entity,entity) => {
    try {
        if(entity.oldWork.mrid === null || entity.oldWork.mrid === '') {
            ...
            const newIdsSet = entity.reactorTestingEquipmentTestType.map(v => v && v.mrid).filter(id => id); // bỏ null/empty
            const oldIdsSet = old_entity.reactorTestingEquipmentTestType.map(v => v && v.mrid).filter(id => id);

            const toAddSet = entity.reactorTestingEquipmentTestType.filter(v => v.mrid && !oldIdsSet.includes(v.mrid));
            const toDeleteSet = old_entity.reactorTestingEquipmentTestType.filter(v => v.mrid && !newIdsSet.includes(v.mrid));
            const toUpdateSet = entity.reactorTestingEquipmentTestType.filter(v => v.mrid && oldIdsSet.includes(v.mrid));
            ...
            const auditResult = await writeJobSaveAuditLog('Reactor Job', old_entity, entity);
            return { success: true, data: entity, changed: auditResult.changed, message: 'Reactor Job entity inserted successfully' };
            ...
            if(dataOldWork.success) {
                ...
                for(const equipment of entity.testingEquipment) {
                    ...
                    if(dataEquipmentTestType.success) {
                        entity.reactorTestingEquipmentTestType = entity.reactorTestingEquipmentTestType.concat(dataEquipmentTestType.data);
                    }
                }
...
export const deleteReactorJobEntity = async (entity) => {
    try {
        ...
        await writeJobDeleteAuditLog('Reactor Job', entity);
