...
export const insertVoltageTransformerJobEntity = async (old_entity,entity) => {
    try {
        if(entity.oldWork.mrid === null || entity.oldWork.mrid === '') {
            ...
            const newIdsSet = entity.voltageTransformerTestingEquipmentTestType.map(v => v && v.mrid).filter(id => id); // bỏ null/empty
            const oldIdsSet = old_entity.voltageTransformerTestingEquipmentTestType.map(v => v && v.mrid).filter(id => id);
            ...
            const auditResult = await writeJobSaveAuditLog('Voltage Transformer Job', old_entity, entity);
            return { success: true, data: entity, changed: auditResult.changed, message: 'Voltage Transformer Job entity inserted successfully' };
...
export const deleteVoltageTransformerJobEntity = async (entity) => {
    try {
        ...
        if(entity.assessment_group && entity.assessment_group.length > 0) {
            for(const item of entity.assessment_group) {
                await deleteAssessmentGroupByIdTransaction(item.mrid, db);
            }
        }
        ...
        await writeJobDeleteAuditLog('Voltage Transformer Job', entity);
