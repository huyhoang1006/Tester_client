...
export const insertRotatingMachineJobEntity = async (old_entity,entity) => {
    try {
        if(entity.oldWork.mrid === null || entity.oldWork.mrid === '') {
            ...
            const newIdsSet = entity.rotatingMachineTestingEquipmentTestType.map(v => v && v.mrid).filter(id => id); // bỏ null/empty
            const oldIdsSet = old_entity.rotatingMachineTestingEquipmentTestType.map(v => v && v.mrid).filter(id => id);

            const toAddSet = entity.rotatingMachineTestingEquipmentTestType.filter(v => v.mrid && !oldIdsSet.includes(v.mrid));
            const toDeleteSet = old_entity.rotatingMachineTestingEquipmentTestType.filter(v => v.mrid && !newIdsSet.includes(v.mrid));
            const toUpdateSet = entity.rotatingMachineTestingEquipmentTestType.filter(v => v.mrid && oldIdsSet.includes(v.mrid));
            ...
            const auditResult = await writeJobSaveAuditLog('Rotating Machine Job', old_entity, entity);
            return { success: true, data: entity, changed: auditResult.changed, message: 'Rotating Machine Job entity inserted successfully' };
        ...
        if(id == null || id === '') {
            ...
            if(dataOldWork.success) {
                ...
                for(const equipment of entity.testingEquipment) {
                    const dataEquipmentTestType = await getRotatingMachineTestingEquipmentTestingEqId(equipment.mrid);
                    if(dataEquipmentTestType.success) {
                        entity.rotatingMachineTestingEquipmentTestType = entity.rotatingMachineTestingEquipmentTestType.concat(dataEquipmentTestType.data);
                    }
                }
...
export const deleteRotatingMachineJobEntity = async (entity) => {
    try {
        ...
        await writeJobDeleteAuditLog('Rotating Machine Job', entity);
