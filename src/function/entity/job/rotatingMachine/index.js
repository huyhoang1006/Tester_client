...
export const insertRotatingMachineJobEntity = async (old_entity,entity) => {
    try {
        if(entity.oldWork.mrid === null || entity.oldWork.mrid === '') {
            ...
            const newIdsSet = entity.rotatingMachineTestingEquipmentTestType.map(v => v.mrid).filter(id => id); // bỏ null/empty
            const oldIdsSet = old_entity.rotatingMachineTestingEquipmentTestType.map(v => v.mrid).filter(id => id);
