...
export const insertBushingJobEntity = async (old_entity,entity) => {
    try {
        if (entity.oldWork.mrid === null || entity.oldWork.mrid === '') {
            ...
            const oldIdsSet = old_entity.bushingTestingEquipmentTestType.map(v => v.mrid).filter(id => id);

            const toAddSet = entity.bushingTestingEquipmentTestType.filter(v => v.mrid && !oldIdsSet.includes(v.mrid));
            const toDeleteSet = old_entity.bushingTestingEquipmentTestType.filter(v => v.mrid && !newIdsSet.includes(v.mrid));
            const toUpdateSet = entity.bushingTestingEquipmentTestType.filter(v => v.mrid && oldIdsSet.includes(v.mrid));
        ...
        if (id == null || id === '') {
            ...
            if (dataOldWork.success) {
                ...
                for (const equipment of entity.testingEquipment) {
                    const dataEquipmentTestType = await getBushingTestingEquipmentTestTypeByTestingEquipmentId(equipment.mrid);
                    if (dataEquipmentTestType.success) {
                        entity.bushingTestingEquipmentTestType = entity.bushingTestingEquipmentTestType.concat(dataEquipmentTestType.data);
                    }
                }
