...
export const insertTransformerJobEntity = async (old_entity,entity) => {
    try {
        if(entity.oldWork.mrid === null || entity.oldWork.mrid === '') {
            ...
            const newIdsSet = entity.transformerTestingEquipmentTestType.map(v => v && v.mrid).filter(id => id); // bỏ null/empty
            const oldIdsSet = old_entity.transformerTestingEquipmentTestType.map(v => v && v.mrid).filter(id => id);

            const toAddSet = entity.transformerTestingEquipmentTestType.filter(v => v.mrid && !oldIdsSet.includes(v.mrid));
            const toDeleteSet = old_entity.transformerTestingEquipmentTestType.filter(v => v.mrid && !newIdsSet.includes(v.mrid));
            const toUpdateSet = entity.transformerTestingEquipmentTestType.filter(v => v.mrid && oldIdsSet.includes(v.mrid));
            ...
            const auditResult = await writeJobSaveAuditLog('Transformer Job', old_entity, entity);
            return { success: true, data: entity, changed: auditResult.changed, message: 'Transformer Job entity inserted successfully' };
        ...
        if(id == null || id === '') {
            ...
            if(dataOldWork.success) {
                ...
                for(const equipment of entity.testingEquipment) {
                    const dataEquipmentTestType = await getTransformerTestingEquipmentTestingEqId(equipment.mrid);
                    if(dataEquipmentTestType.success) {
                        entity.transformerTestingEquipmentTestType = entity.transformerTestingEquipmentTestType.concat(dataEquipmentTestType.data);
                    }
                }
...
export const deleteTransformerJobEntity = async (entity) => {
    try {
        ...
        await writeJobDeleteAuditLog('Transformer Job', entity);
