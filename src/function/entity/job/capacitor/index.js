...
export const insertCapacitorJobEntity = async (old_entity,entity) => {
    try {
        if(entity.oldWork.mrid === null || entity.oldWork.mrid === '') {
            ...
            const newIdsSet = entity.capacitorTestingEquipmentTestType.map(v => v && v.mrid).filter(id => id); // bỏ null/empty
            const oldIdsSet = old_entity.capacitorTestingEquipmentTestType.map(v => v && v.mrid).filter(id => id);

            const toAddSet = entity.capacitorTestingEquipmentTestType.filter(v => v.mrid && !oldIdsSet.includes(v.mrid));
            const toDeleteSet = old_entity.capacitorTestingEquipmentTestType.filter(v => v.mrid && !newIdsSet.includes(v.mrid));
            const toUpdateSet = entity.capacitorTestingEquipmentTestType.filter(v => v.mrid && oldIdsSet.includes(v.mrid));
            ...
            const auditResult = await writeJobSaveAuditLog('Capacitor Job', old_entity, entity);
            return { success: true, data: entity, changed: auditResult.changed, message: 'Capacitor Job entity inserted successfully' };
        ...
        if(id == null || id === '') {
            ...
            if(dataOldWork.success) {
                ...
                for(const equipment of entity.testingEquipment) {
                    const dataEquipmentTestType = await getCapacitorTestingEquipmentTestingEqId(equipment.mrid);
                    if(dataEquipmentTestType.success) {
                        entity.capacitorTestingEquipmentTestType = entity.capacitorTestingEquipmentTestType.concat(dataEquipmentTestType.data);
                    }
                }
...
export const deleteCapacitorJobEntity = async (entity) => {
    try {
        ...
        await writeJobDeleteAuditLog('Capacitor Job', entity);
