...
export const insertSurgeArresterJobEntity = async (old_entity,entity) => {
    try {
        if(entity.oldWork.mrid === null || entity.oldWork.mrid === '') {
            ...
            const newIdsSet = entity.surgeArresterTestingEquipmentTestType.map(v => v && v.mrid).filter(id => id); // bỏ null/empty
            const oldIdsSet = old_entity.surgeArresterTestingEquipmentTestType.map(v => v && v.mrid).filter(id => id);
