...
export const insertDisconnectorJobEntity = async (old_entity,entity) => {
    try {
        if(entity.oldWork.mrid === null || entity.oldWork.mrid === '') {
            ...
            const newIdsSet = entity.disconnectorTestingEquipmentTestType.map(v => v && v.mrid).filter(id => id); // bỏ null/empty
            const oldIdsSet = old_entity.disconnectorTestingEquipmentTestType.map(v => v && v.mrid).filter(id => id);

            const toAddSet = entity.disconnectorTestingEquipmentTestType.filter(v => v.mrid && !oldIdsSet.includes(v.mrid));
            const toDeleteSet = old_entity.disconnectorTestingEquipmentTestType.filter(v => v.mrid && !newIdsSet.includes(v.mrid));
            const toUpdateSet = entity.disconnectorTestingEquipmentTestType.filter(v => v.mrid && oldIdsSet.includes(v.mrid));
        ...
        if(id == null || id === '') {
            ...
            if(dataOldWork.success) {
                ...
                for(const equipment of entity.testingEquipment) {
                    const dataEquipmentTestType = await getDisconnectorTestingEquipmentTestingEqId(equipment.mrid);
                    if(dataEquipmentTestType.success) {
                        entity.disconnectorTestingEquipmentTestType = entity.disconnectorTestingEquipmentTestType.concat(dataEquipmentTestType.data);
                    }
                }
